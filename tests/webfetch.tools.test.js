...
test('Agent-cli webfetch tool produces OpenCode-compatible JSON output', async () => {
  // Test our agent-cli webfetch tool (compatible with OpenCode format)
  ...
  console.log('Expected OpenCode JSON event structure for webfetch tool:')
  ...
  const agentToolEvents = agentEvents.filter(e => e.type === 'tool_use' && e.part.tool === 'webfetch')
  ...
  expect(typeof agentTool.part.state.input.url).toBeTruthy()
  expect(typeof agentTool.part.state.input.format).toBeTruthy()
  ...
  // Verify the content was fetched (should contain HTML)
  expect(agentTool.part.state.output.includes('<html>') || agentTool.part.state.output.includes('Herman Melville')).toBeTruthy()

  console.log('✅ Webfetch tool test passed - agent-cli produces OpenCode-compatible JSON format')
